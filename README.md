...
- Ubuntu 服务器（已安装 **LightDM** + **xinit** 并配置自动登录）
...
```bash
cd F:\Ai-agent\fan_control\install
chmod +x deploy.sh
...
```bash
# 1. 上传文件
scp fan_control.sh wangxian@192.168.2.167:/home/fan_control/
scp nvidia-fan-helper wangxian@192.168.2.167:/usr/local/bin/
scp fan-control.service wangxian@192.168.2.167:~/.config/systemd/user/
scp xdiag/xdiag.sh wangxian@192.168.2.167:/home/fan_control/xdiag/

# 2. 设置权限
ssh wangxian@192.168.2.167
sudo chmod 755 /home/fan_control/fan_control.sh
sudo chmod 755 /usr/local/bin/nvidia-fan-helper
sudo chmod 755 /home/fan_control/xdiag/xdiag.sh
...
**根因：** fan-control.service（user systemd 单元）的启动时间比 lightdm 自动登录早了约 4 秒。此时 gnome-session 还未将 DISPLAY/XAUTHORITY 注入 systemd --user 环境。进程 fork 时环境变量为空且永不更新，导致 `detect_display()` 所有路径均失败。
