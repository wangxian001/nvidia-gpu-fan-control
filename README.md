...
本系统的认证机制基于 **SI:localuser**（Server Interpreted: local user）。当本地用户直接从本地连接 X server（不经过 SSH），X server 会通过该机制自动放行认证，无需 `.Xauthority` 文件。

```
本地用户身份 → 本机进程 → X server → SI:localuser = 放行
root 身份     → sudo 后进程 → X server → SI:localuser = 拒绝！
                             （root 不在 SI 列表中）
```
...
```bash
# 1. 在服务器创建目录结构
ssh your-username@your-server-ip "sudo mkdir -p /home/fan_control/xdiag ~/.config/systemd/user"

# 2. 上传文件（nvidia-fan-helper 放到 /tmp 再用 sudo 移入）
scp fan_control.sh         your-username@your-server-ip:/tmp/
scp nvidia-fan-helper      your-username@your-server-ip:/tmp/
scp fan-control.service    your-username@your-server-ip:/tmp/
scp xdiag/xdiag.sh         your-username@your-server-ip:/tmp/
...
sudo chown $USER:$USER /home/fan_control/fan_control.sh

cp /tmp/xdiag.sh /home/fan_control/xdiag/xdiag.sh
chmod 755 /home/fan_control/xdiag/xdiag.sh

cp /tmp/fan-control.service ~/.config/systemd/user/fan-control.service
...
2. **`nvidia-fan-helper` 必须在 sudoers 中配置 NOPASSWD**
   ...
   # 添加一行:
   <your-username> ALL=(ALL) NOPASSWD: /usr/local/bin/nvidia-fan-helper
   ```
...
2. **去掉 `get_display_v` 和 `reset_auto_d` 的 `sudo`**（非必需，但增加了鲁棒性）—— 去掉后 helper 以普通用户身份运行，可通过 SI:localuser 机制通过 X 认证
